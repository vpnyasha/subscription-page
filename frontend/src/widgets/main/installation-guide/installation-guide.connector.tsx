...
import { showCopyNotification } from '@shared/constants'
...
/**
 * Иконки платформ приходят из панели готовой SVG-разметкой, поэтому вставляем
 * их как есть. Обёртка задаёт размер: сами иконки приходят без него.
 */
function PlatformIcon({ svg }: { svg: string }) {
    return (
        <span
    ...
    const handleButtonClick = async (button: TSubscriptionPageButtonConfig) => {
        ...
        switch (button.type) {
            case 'copyButton': {
                ...
                if (await copyText(formattedUrl)) {
        ...
        <Card p={{ base: 'sm', xs: 'md', sm: 'lg', md: 'xl' }} radius="lg">
            <Stack gap="md">
                <Group gap="sm" justify="space-between">
                    ...
                    {/* Свой дропдаун вместо NativeSelect: нативный список на
                        телефоне рисует система, и иконки платформ в нём теряются
                        — остаются одни названия. Иконки те же, из svgLibrary
                        панели, что и в закрытой кнопке. */}
                    {availablePlatforms.length > 1 && (
                        <Menu
                            classNames={{ dropdown: classes.platformDropdown }}
                            position="bottom-end"
                            radius="md"
                            width={180}
                            withArrow={false}
                        >
                            <Menu.Target>
                                <Button
                                    classNames={{
                                        ...
                                        label: classes.platformSelectLabel
                                    }}
                                    ...
                                    /* Menu.Target объединяет свой обработчик
                                       с нашим, так что список всё равно
                                       раскрывается. */
                                    onClick={() => vibrate('tap')}
                                    radius="md"
                            ...
                            <Menu.Dropdown>
                                {availablePlatforms.map((opt) => (
                                    <Menu.Item
                                        key={opt.value}
                                        leftSection={<PlatformIcon svg={opt.icon} />}
                                        onClick={() => {
                                            vibrate([80])
